...
namespace GtkFlow {
    ...
    public class Minimap : Gtk.DrawingArea {
        ...
        public Minimap() {
            ...
            this.clickcontroller.pressed.connect((n,x,y)=>{ this.do_button_release_event(n,x,y); });
            this.clickcontroller.released.connect((n,x,y)=>{ this.do_button_press_event(n,x,y); });
        }

        private bool do_button_press_event(int n, double x, double y) {
        ...
        private new void snapshot(Gtk.Snapshot sn) {
            ...
            var alloc_rect = Graphene.Rect.alloc().init(own_alloc.x, own_alloc.y, own_alloc.width, own_alloc.height);
